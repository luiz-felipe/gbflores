...
public class JGnuplotDemo extends Demo {
	...
	public void prepPlot() {
		...
		for (double i = -2*Math.PI; i <= 2*Math.PI; i += 0.2) {
			for (double j = -2*Math.PI; j <= 2*Math.PI; j += 0.2) {
				x.add(i);
				y.add(j);
				z1.add(10*Math.sin(i));
				z2.add(Math.cos(i)+1);
			}
		}
	...
	public void execute() {
		...
		jg.execute(plot1, jg.plot2d, jg.JG_Pause);
		jg.execute(plot2, jg.plot3d, jg.JG_Pause);
