...
public class CopyOfAtividade2 {
	
	public static void main(String[] args) throws Exception {
		new CopyOfAtividade2();	}

	public CopyOfAtividade2() throws Exception {
...
//		VLEExperiment vle = new VLEExperiment("Hydrogen + Toluene T at 302.0 C.txt");
		...
		double deltaX = vle.getPoints().get(vle.getPoints().size()-1).getX()[0] - 
				vle.getPoints().get(0).getX()[0];
		
		double xx[] = new double [150];
		double deltaDeltaX = deltaX/(xx.length+0D);
		for (int i = 0; i < xx.length; i++) {
			xx[i] = vle.getPoints().get(0).getX()[0]+deltaDeltaX*i;
		}
		double y[] ={0.7, 0.3};
		for (int i = 0; i < xx.length; i++) {

			T = vle.getPoints().get(0).T;
			P = vle.getPoints().get(0).P;
			double x[] = {xx[i], (1-xx[i])};
			...
			while (Math.abs(oldP - P) > 1e-10) {
				...
				while (Math.abs(sumKx - oldSumKx) > 1e-10) {
					...
					for (int ii = 0; ii < K.length; ii++) {
						K[ii] = fugliq[ii] / fugvap[ii];
						sumKx += K[ii] * x[ii];
					}
					for (int ii = 0; ii < K.length; ii++)
						y[ii] = K[ii] * x[ii] / sumKx;
